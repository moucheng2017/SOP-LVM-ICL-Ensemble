...
# With options to add more prompts including:
# 1. Intent
# 2. Metadata
...
def extract_and_format_json(json_file_path):
    ...
    for i, entry in enumerate(data, start=1):
        ...
        mouse_button = entry.get("Mouse_Button")
        key_pressed = entry.get("Key_Pressed")        
        # Step 3: Format the extracted information
        if key_pressed == None:
            formatted_entry = f"{i}. Mouse Position: {position}, Mouse Button: {mouse_button}"
        else:
            formatted_entry = f"{i}. Mouse Position: {position}, Mouse Button: {mouse_button}, Key Pressed: {key_pressed}."
...
def main_gemini(args):
    ...
    generation_config_reflect = GenerationConfig(
    ...
    if config["in_context_learning"] == True: 
        ...
        for video in train_videos_paths_:
            ...
            # Add the intent to the promp, the intent is the first line of the labels
            if config['intent'] and config['intent'] == True:
                # Add the intent to the prompt:
                prompt.append({
                    "role": "user",
                    "parts": f"The task of the above screenshots is for: {labels[0]}"
                })
            ...
            labels = [line for line in labels if line.strip() != '']
            labels = '\n'.join(labels)
            labels_header = "The SOP labels of the above screenshots are: \n"
            ...
            # Add meta data to the prompt
            if config['metadata'] and config['metadata'] == True:
                # Loop through the meta data and add them in a template txt:
                meta_data = "The following meta data is provided for the above screenshots: \n"
                meta_data += extract_and_format_json(video + '/metadata.json')
                prompt.append({
                    "role": "user",
                    "parts": meta_data  
                })
        ...
        for n in range(num_inferences):
            for try_time in range(5): # Try 5 times
                try:                    
                    ...
                    # Add the intent to the prompt if it is enabled
                    if config['intent'] and config['intent'] == True:
                        # Read the labels and add the intent to the prompt:
                        labels = read_labels(video)
                        # remove first line:
                        labels = labels.split('\n') 
                        # remove empty lines: 
                    ...
                    # Add the metadata to the prompt if it is enabled
                    if config['metadata'] and config['metadata'] == True:
                        # Loop through the meta data and add them in a template txt:
                        meta_data = "The following meta data is provided for the above screenshots: \n"
                        meta_data += extract_and_format_json(video + '/metadata.json')
            ...
            for try_time in range(10): # Try 5 times
                try:                    
                    response = model.generate_content(reflection_prompt,
                                                      generation_config=generation_config_reflect,
                                                      safety_settings=safe,
                                                      request_options={"timeout": 2000})
                    break
                except Exception as e:
