...
def resize_image_by_scale_factor(image_path: str, scale_factor: float, save_path: str):
    # Open the image
...
def _fetch_geminipro_completion(messages: List[Any], model_name: str) -> str:
    ...
    for idx, message in enumerate(messages):
        for content_obj in message["content"]:
            ...
            if content_obj["type"] == "text":
                ...
                path_to_tmp_file: str = f'/tmp/workflows/img_{random.randint(0, 9999999)}.png'
                img.save(path_to_tmp_file)
                # Upload file to Gemini Files API
                file = genai.upload_file(path=path_to_tmp_file)
                content.append(file)
                # if len(img_data) >= 10_000_000:
                    # Limit to 20 MB per prompt, so downsample image if necessary
                # content.append(img)
                img_counter += 1
            else:
                raise ValueError(f"Unknown content type: {content_obj['type']}")
    assert len(content) + n_skipped_images == n_message_contents, f"Length mismatch: {len(content)} != {n_message_contents}"
    new_messages = [{
        'role' : 'user',
        'parts' : content,
    }]
    ...
    try:
        response = model.generate_content(new_messages, safety_settings={'harassment': 'block_none', 
...
def _fetch_claude_completion(messages: List[Any], model_name: str) -> str:
            ...
            if content_obj['type'] == 'text':
                ...
                content.append({
                    ...
                    'source' : {
                        ...
                        'data' : content_obj['image_url']['url'][22:], # First 22 characters are "data:image/png;base64,",
                    }
                })
                img_counter += 1
            else:
    ...
    try:
        ...
        if "rate limit" in str(e):
            print(f"Rate limit exceeded -- waiting 1 min before retrying")
            time.sleep(60)
            return _fetch_claude_completion(messages, model_name)
        if "too many images" in str(e):
            breakpoint()
        traceback.print_exc()
        print(f"Unknown error: {e}")
...
def filter_prompt_s_a_sequence(prompt_s_a_sequence: List[str], is_kf: bool, is_act: bool, paths_to_screenshots: List[str] = []) -> List[str]:
    """If is_kf, then keep screenshots; if is_act, then keep actions"""
    if not is_kf:
        # remove screenshots
        prompt_s_a_sequence = [ x for x in prompt_s_a_sequence if x['content'][0]['type'] != 'image_url' ]
        paths_to_screenshots = []
    if not is_act:
         # remove actions
        prompt_s_a_sequence = [ x for x in prompt_s_a_sequence if x['content'][0]['type'] != 'text' ]
